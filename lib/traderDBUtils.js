module.exports = {
  ...
  getBTChinaCredentials: function(traderID, mysqlConn, callback){
    var query = "SELECT btcchinaKey, btcchinaSecret FROM borrower WHERE borrower_id='" + traderID + "';";
    mysqlConn.query(query, function(results){
      callback(results[0]);
    });
  },

  getContactEmail: function(traderID, mysqlConn, callback){
    var query = "SELECT email FROM borrower WHERE borrower_id='" + traderID + "';";
    mysqlConn.query(query, function(results){
      callback(results[0].email);
    });
  },

  getNotificationThreshold: function(traderID, mysqlConn, callback){
    var query = "SELECT notification_threshold FROM borrower WHERE borrower_id='" + traderID + "';";
    mysqlConn.query(query, function(result){
      var notificationThresh = result[0].notification_threshold;
      callback(notificationThresh);
    });
  },
